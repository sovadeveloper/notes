package com.sovadeveloper.notes.services.Impl;

import com.sovadeveloper.notes.entities.NoteEntity;
import com.sovadeveloper.notes.repositories.NoteRepo;
import com.sovadeveloper.notes.services.NoteService;
import lombok.RequiredArgsConstructor;
import org.springframework.stereotype.Service;

import java.util.List;

@Service
@RequiredArgsConstructor
public class NoteServiceImpl implements NoteService {
    private final NoteRepo noteRepo;

    @Override
    public NoteEntity create(NoteEntity noteEntity) throws Exception {
        if(noteEntity.getTitle().length() < 1){
            throw new Exception("Название заметки не может быть пустым");
        }
        if(noteEntity.getText().length() < 1){
            throw new Exception("Содержание заметки не может быть пустым");
        }
        return noteRepo.save(noteEntity);
    }

    @Override
    public NoteEntity getById(Long id) throws Exception {
        return noteRepo.findById(id)
                .orElseThrow(() -> new Exception("Такой заметки не найдено"));
    }

    @Override
    public NoteEntity edit(NoteEntity noteEntityUpdated) throws Exception {
        if(noteEntityUpdated.getTitle().length() < 1){
            throw new Exception("Название заметки не может быть пустым");
        }
        if(noteEntityUpdated.getText().length() < 1){
            throw new Exception("Содержание заметки не может быть пустым");
        }
        return noteRepo.save(noteEntityUpdated);
    }

    @Override
    public Long delete(Long id) throws Exception {
        NoteEntity noteEntity = noteRepo.findById(id)
                .orElseThrow(() -> new Exception("Такой заметки не найдено"));
        noteRepo.delete(noteEntity);
        return id;
    }

    @Override
    public List<NoteEntity> getAll() throws Exception {
        return noteRepo.findAll();
    }
}
